...
def records_to_graph():
    ...
    for director in directors:
      new_node = Node(node_count, director, 'director')
      nodelist.append(new_node)
      nodedict[new_node.name] = new_node
      node_count += 1

    for actor in actors:
      new_node = Node(node_count, actor, 'actor')
      nodelist.append(new_node)
      nodedict[new_node.name] = new_node
      node_count += 1

    for genre in genres:
      new_node = Node(node_count, genre, 'genre')
      nodelist.append(new_node)
      nodedict[new_node.name] = new_node
      node_count += 1

    for movie in movies:
      new_node = Node(node_count, movie, 'movie')
      nodelist.append(new_node)
      nodedict[new_node.name] = new_node
      node_count += 1
    ...
    for movie in movies:
      for rating in range(5):
        new_node = Node(node_count, movie+'_'+str(rating+1), 'rating')
        nodelist.append(new_node)
        nodedict[new_node.name] = new_node
        node_count += 1
